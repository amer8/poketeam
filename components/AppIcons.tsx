...
function getIconClassName(className?: string) {
  return className ? `${styles.icon} ${className}` : styles.icon;
}

export function DarkModeIcon({ className, ...props }: AppIconProps) {
  return (
    <svg
      aria-hidden="true"
...
export function CheckCircleIcon({ className, ...props }: AppIconProps) {
  return (
    <svg
      aria-hidden="true"
      className={getIconClassName(className)}
